...
import {
  MapPin, Star, Eye, Heart, Phone, Mail, Globe, ArrowLeft,
  Tent, Leaf, Share2, Calendar, ChevronLeft, ChevronRight,
} from 'lucide-react';
import { formatPrice, getPlaceTypeLabel, getRelativeTime, cn } from '@/lib/utils';
...
export default function PlaceDetailClient({ place, initialLiked, profile }: PlaceDetailClientProps) {
  ...
  async function handleLike() {
    try {
      ...
      setLiked(result);
      setLikeCount((c) => c + (result ? 1 : -1));
      toast.success(result ? '❤️ Хадгаллаа' : 'Хасагдлаа');
    } catch {
  ...
  return (
    <div className="min-h-screen bg-cream">
      ...
      <div className="page-container pt-8 pb-4">
        <Link href="/" className="inline-flex items-center gap-2 text-forest-500 text-sm hover:text-forest-700 transition-colors">
      ...
      <div className="page-container mb-8">
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-3 h-96 lg:h-[28rem]">
          {/* Main image */}
          <div className="relative lg:col-span-3 rounded-2xl overflow-hidden bg-forest-100 cursor-pointer"
               onClick={() => setImageIndex(0)}>
            {allImages[imageIndex] ? (
              <Image src={allImages[imageIndex]} alt={place.name} fill className="object-cover" priority />
            ) : (
              <div className="absolute inset-0 flex items-center justify-center">
                {isResort ? <Tent size={64} className="text-forest-300" /> : <Leaf size={64} className="text-forest-300" />}
              </div>
            )}
            ...
            {allImages.length > 1 && (
              <>
                ...
                <button
                  onClick={(e) => { e.stopPropagation(); setImageIndex((i) => Math.min(allImages.length - 1, i + 1)); }}
                ...
                <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
                  {allImages.map((_, i) => (
                    <button key={i} onClick={(e) => { e.stopPropagation(); setImageIndex(i); }}
                      className={cn('w-2 h-2 rounded-full transition-all', i === imageIndex ? 'bg-white w-5' : 'bg-white/50')} />
                  ))}
                </div>
              </>
            )}
          ...
          <div className="hidden lg:grid lg:col-span-2 grid-rows-2 gap-3">
            {allImages.slice(1, 3).map((img, i) => (
              <div key={i} className="relative rounded-2xl overflow-hidden bg-forest-100 cursor-pointer"
                   onClick={() => setImageIndex(i + 1)}>
                <Image src={img} alt={`${place.name} ${i + 2}`} fill className="object-cover hover:scale-105 transition-transform duration-300" />
              </div>
            ))}
            {allImages.length > 3 && (
              <div className="relative rounded-2xl overflow-hidden bg-forest-100 cursor-pointer"
                   onClick={() => setImageIndex(3)}>
                <Image src={allImages[3]} alt="" fill className="object-cover" />
          ...
          <div className="lg:col-span-2">
            ...
            <div className="flex items-start justify-between gap-4 mb-6">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <button className="p-2.5 rounded-xl border border-forest-200 bg-white text-forest-500 hover:text-forest-700 transition-colors">
                  <Share2 size={15} />
                </button>
              </div>
            </div>
                  ...
                  <div className="flex">
                    {[1,2,3,4,5].map((s) => (
                      <Star key={s} size={16} className={cn(
                        s <= Math.round(place.rating_avg) ? 'text-amber-400 fill-amber-400' : 'text-forest-200 fill-forest-200'
                      )} />
                    ))}
                  </div>
                  <span className="font-semibold text-forest-800">{place.rating_avg.toFixed(1)}</span>
                  <span className="text-forest-500 text-sm">({place.rating_count} үнэлгээ)</span>
                </div>
              )}
              <div className="flex items-center gap-1.5 text-forest-500 text-sm">
                <Eye size={15} /> {place.view_count.toLocaleString()} үзэлт
              </div>
            ...
            {/* Description */}
            <div className="mb-10">
              <h2 className="font-display text-2xl font-semibold text-forest-900 mb-4">Тайлбар</h2>
              <div className="rich-text text-forest-600 leading-relaxed">
            ...
            {(place.phone || place.email || place.website) && (
              <div className="mb-10">
                <h2 className="font-display text-2xl font-semibold text-forest-900 mb-4">Холбоо барих</h2>
                  ...
                  {place.phone && (
                    <a href={`tel:${place.phone}`}
                      ...
                      <div>
                        ...
                        <div className="text-sm font-medium text-forest-800">{place.phone}</div>
                      </div>
                    </a>
                  )}
                  {place.email && (
                    <a href={`mailto:${place.email}`}
                      ...
                      <div>
                        ...
                        <div className="text-sm font-medium text-forest-800 truncate">{place.email}</div>
                      </div>
                    </a>
                  )}
                  {place.website && (
                    <a href={place.website} target="_blank" rel="noopener noreferrer"
                      ...
                      <div>
                        ...
                        <div className="text-sm font-medium text-forest-800 truncate">Нээх</div>
                      </div>
                    </a>
                  )}
                </div>
            ...
            {place.latitude && place.longitude && (
              <div className="mb-10">
                <h2 className="font-display text-2xl font-semibold text-forest-900 mb-4">Байршил</h2>
                ...
                >
                  <MapPin size={15} /> Google Maps-ээр очих заалт авах
                </a>
              </div>
            )}
