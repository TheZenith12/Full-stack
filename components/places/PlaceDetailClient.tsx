...
import MobileBookingDrawer from "@/components/places/MobileBookingDrawer";
import ReviewsSection from "@/components/places/ReviewsSection";
import type { Place, Profile, Review } from "@/lib/types";

// Place-д reviews relation нэмсэн өргөтгөсөн тип
type PlaceWithReviews = Place & {
...
export default function PlaceDetailClient({
  ...
  function handleLike() {
    ...
    likeDebounceRef.current = setTimeout(async () => {
      ...
      if (nextLiked) {
        await supabase.from("likes").upsert({ user_id: profile.id, place_id: place.id } as never);
      } else {
        await supabase.from("likes").delete().eq("user_id", profile.id).eq("place_id", place.id);
      }
    }, 600);
  }
  ...
  return (
    <div className="min-h-screen bg-cream">
      ...
      <MobileBookingDrawer place={place} profile={profile} />
    </div>
  );
}
